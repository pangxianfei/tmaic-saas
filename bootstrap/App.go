...
func RouteNameList(app *iris.Application, AppName string, Port string, AppId int64) {
	...
	for _, value := range routeList {
		...
		if value.Method == "POST" || value.Method == "GET" {
			...
			if Authority.Id > 0 {
				...
				if isAuthority.Id <= 0 {
					createAuthority := &SysAppModel.Authority{
						Pid:              Authority.Id,
						AppId:            AppId,
						Name:             simple.Split(value.MainHandlerName, "."),
						Description:      value.Description,
			...
			console.Println(console.CODE_SUCCESS, " "+console.Sprintf(console.CODE_SUCCESS, "%-4d", index)+console.Sprintf(console.CODE_SUCCESS, "%-40s", value.MainHandlerName)+console.Sprintf(console.CODE_SUCCESS, "%-5s", value.Method)+" "+console.Sprintf(console.CODE_SUCCESS, "%-45s", value.Path)+console.Sprintf(console.CODE_SUCCESS, "%-30s", value.StaticPath())+console.Sprintf(console.CODE_SUCCESS, "%-20s", value.RegisterFileName))
			index++
		}
	}
